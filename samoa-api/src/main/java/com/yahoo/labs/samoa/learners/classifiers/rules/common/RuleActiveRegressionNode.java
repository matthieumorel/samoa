...
public class RuleActiveRegressionNode extends RuleRegressionNode implements RuleActiveLearningNode {
    ...
    public void updateStatistics(Instance inst) {
        ...
        for (int i = 0; i < inst.numAttributes() - 1; i++) {
            ...
            if (obs != null) {
                ((FIMTDDNumericAttributeClassObserver) obs).observeAttributeClass(inst.value(instAttIndex),
    ...
    public boolean tryToExpand(double splitConfidence, double tieThreshold) {
        ...
        // Declare a variable to determine if any of the splits should be
        // performed
        ...
        } // Otherwise, consider which of the splits proposed may be worth
          // trying
        else {
            // Determine the hoeffding bound value, used to select how many
            // instances should be used to make a test decision
            // to feel reasonably confident that the test chosen by this sample
            // is the same as what would be chosen using infinite examples
            ...
            // as determined using the hoeffding bound, is less than 1, then the
            // true mean is also less than 1, and thus at this
            // particular moment of observation the bestSuggestion is indeed the
            // best split option with confidence 1-delta, and
            // splitting should occur.
            // Alternatively, if two or more splits are very similar or
            // identical in terms of their splits, then a threshold limit
            // (default 0.05) is applied to the hoeffding bound; if the
            // hoeffding bound is smaller than this limit then the two
            // competing attributes are equally good, and the split will be made
            // on the one with the higher SDR value.
    ...
    public AttributeSplitSuggestion[] getBestSplitSuggestions(SplitCriterion criterion) {
        ...
        // Set the nodeStatistics up as the preSplitDistribution, rather than
        // the observedClassDistribution
