...
public class CMM extends MeasureCollection {
        ...
        for (int p = 0; p < numPoints; p++) {
            ...
            for (int fc = 0; fc < numFClusters; fc++) {
                ...
                if (pointInclusionProbFC[p][fc] >= pointInclusionProbThreshold) {
                    // make sure we don't count points twice that are contained
                    // in two merged clusters
                    if (cmdp.isNoise())
                        continue;
    ...
    private void calculateError() {
         ...
         * point. within the same run calculate various other stuff like
         * coverage etc...
         */
        for (int p = 0; p < numPoints; p++) {
            CMMPoint cmdp = gtAnalysis.getPoint(p);
            ...
            if (coverage > 0 && !cmdp.isNoise())
                trueCoverage++; // points covered by clustering, don't count
                                // noise
            if (coverage > 1)
    ...
    private double noiseError(CMMPoint cmdp, int assignedClusterID) {
        ...
        // TODO: Noiscluster?
        // also happens when we decrease the radius and there is only a noise
        // point in the center
        if (gtAssignedID == -1) {
            error = 1;
    ...
    private double missedError(CMMPoint cmdp, boolean useHullDistance) {
        ...
        if (!useHullDistance) {
            ...
            for (int fc = 0; fc < numFClusters; fc++) {
                // if fc is mappend onto the class of the point, check it for
                // its hulldist
    ...
    private double misplacedError(CMMPoint cmdp, int assignedClusterID) {
        ...
        if (gtAssignedID == cmdp.workclass())
            ...
            // assigned and real GT0 cluster are not connected, but does the
            // model have the
            ...
            if (enableModelError
                ...
                // point was mapped onto wrong cluster (assigned), so check how
                // far away
