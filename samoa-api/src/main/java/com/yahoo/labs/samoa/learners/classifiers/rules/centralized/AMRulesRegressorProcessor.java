...
public class AMRulesRegressorProcessor implements Processor {
     ...
     * in moa.classifier.java returns the prediction of the instance. Called in
    ...
    public void trainOnInstanceImpl(Instance instance) {
         ...
         * AMRules Algorithm
         * 
         * //For each rule in the rule set //If rule covers the instance //if
         * the instance is not an anomaly //Update Change Detection Tests
         * //Compute prediction error //Call PHTest //If change is detected then
         * //Remove rule //Else //Update sufficient statistics of rule //If
         * number of examples in rule > Nmin //Expand rule //If ordered set then
         * //break //If none of the rule covers the instance //Update sufficient
         * statistics of default rule //If number of examples in default rule is
         * multiple of Nmin //Expand default rule and add it to the set of rules
        ...
        while (ruleIterator.hasNext()) {
            ...
            if (rule.isCovering(instance) == true) {
                ...
                if (isAnomaly(instance, rule) == false) {
                    ...
                    double error = rule.computeError(instance); // Use adaptive
                                                                // mode error
                    boolean changeDetected = ((RuleActiveRegressionNode) rule.getLearningNode())
                            .updateChangeDetection(error);
    ...
    private boolean isAnomaly(Instance instance, ActiveRule rule) {
        // AMRUles is equipped with anomaly detection. If on, compute the
        // anomaly value.
