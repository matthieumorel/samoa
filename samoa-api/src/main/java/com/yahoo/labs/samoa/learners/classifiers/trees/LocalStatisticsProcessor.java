...
final class LocalStatisticsProcessor implements Processor {
    ...
    public boolean process(ContentEvent event) {
        // process AttributeContentEvent by updating the subset of local
        // statistics
        if (event instanceof AttributeBatchContentEvent) {
             ...
             * AttributeContentEvent ace = (AttributeContentEvent) event; Long
             * learningNodeId = Long.valueOf(ace.getLearningNodeId()); Integer
             * obsIndex = Integer.valueOf(ace.getObsIndex());
             * 
             * AttributeClassObserver obs = localStats.get( learningNodeId,
             * obsIndex);
             * 
             * if (obs == null) { obs = ace.isNominal() ?
            ...
            // and send back the calculation results via computation result
            // stream.
